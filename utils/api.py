...
def Despot_Estimate(sptFile, cfg, force=False):
...
def Despot_Benchmark(sptFile, cfg, mode: str = "cluster", force: bool = False):
...
def Pip_decont(sptFile, cfg, method='none', force=False, ):
...
def Pip_cluster(sptFile, cfg, h5data, method='leiden', force=False, tif=None):
    ...
    if method == 'leiden':
        ...
        Save_spt_from_SEDR(sptFile, adata, h5data)
...
def Pip_deconv(sptFile, cfg, h5data='matrix', method="stereoScope", name='temp', force=False, standard_size=10):
...
def Despot_Decont(sptFile, cfg, force=False):
...
def Despot_Cluster(sptFile, cfg, force=False):
    ...
    for h5data in h5datas:
        for method in methods:
            Pip_cluster(sptFile, cfg, h5data, method=method, tif=tif, force=force)


def Despot_Deconv(sptFile, cfg=None, name='temp', force=False):
    ...
    for method in methods:
        for h5data in h5datas:
            ...
            start = time.time()
            Pip_deconv(sptFile, cfg, h5data, method=method, name=name, force=force)
            end = time.time()
...
def Despot_SubsetDeconv(sptFile, h5data="matrix", clu_mtd=None, domain=None):
    adata = Load_spt_to_AnnData(sptFile, h5data)
    ...
    Save_spt_from_configs(sptFile, {"Decontamination": subset_name})
    Despot_Deconv(sptFile)
